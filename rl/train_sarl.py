#!/usr/bin/env python3
"""
Self-play Reinforcement Learning training script for Catan Policy-Value Network.
...
class RLAgent:
    ...
    def select_action(self, state: torch.Tensor, valid_actions: np.ndarray, 
    ...
    def evaluate_actions(self, states: torch.Tensor, actions: torch.Tensor, 
                        valid_actions_batch: List[np.ndarray]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
...
def compute_gae(rewards: np.ndarray, values: np.ndarray, dones: np.ndarray,
                next_value: float, gamma: float = 0.99, gae_lambda: float = 0.95) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def ppo_update(agent: RLAgent, optimizer: optim.Optimizer, buffer: ExperienceBuffer,
               clip_epsilon: float, value_coef: float, entropy_coef: float,
               n_epochs: int, batch_size: int, device: str) -> Dict[str, float]:
    """
...
def train_self_play(
    ...
    """Train PolicyValueNetwork using self-play with PPO."""
    
    print(f"\n{'='*60}")
    print("Training Policy-Value Network with Self-Play (PPO)")
    print(f"{'='*60}")
    ...
    # Create environment (will be recreated for each episode)
    print("\nStarting training...")
    
    for episode in range(n_episodes):
        # Create fresh environment for this episode with configured opponents
        env = gymnasium.make(
        ...
        # Episode collection (model already in eval mode via select_action)
        while not done:
            ...
            state = game_to_features(game, current_player.color)
            
            # Check for NaN in features
            if np.isnan(state).any():
                print(f"WARNING: NaN in features at step {episode_length}")
                # Skip this step or handle gracefully
                continue
            
            state_t = torch.from_numpy(state).float().unsqueeze(0).to(device)
            
            # Get valid actions from info
            valid_actions = np.array(info['valid_actions'], dtype=np.int64)
            
            # Select action
            action_idx, log_prob, value = agent.select_action(state_t, valid_actions)
            
            # Take action in environment
            obs, reward, terminated, truncated, info = env.step(action_idx)
            done = terminated or truncated
            
            # Store experience (ensure float32)
            buffer.add(state.astype(np.float32), action_idx, reward, value, log_prob, valid_actions, done)
        ...
        # Perform PPO update (only if we have enough samples for at least 2 batches)
        min_samples = max(batch_size * 2, 4)  # Need at least 2 full batches or 4 samples
        if (episode + 1) % update_freq == 0 and len(buffer) >= min_samples:
            metrics = ppo_update(
                agent, optimizer, buffer, clip_epsilon, value_coef,
                entropy_coef, n_epochs, batch_size, device
            )
            buffer.clear()
            model.eval()  # Set back to eval mode for next episode
        ...
        # Save best model (only after we have enough episodes for meaningful average)
        min_episodes_for_best = min(update_freq, len(episode_rewards))
        ...
        if (episode + 1) % save_freq == 0:
            # Keep same directory, change filename
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help='Update model every N episodes (default: 32)'
    )
    parser.add_argument(
        '--lr',
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help='Mini-batch size for PPO update (default: 64)'
    )
    parser.add_argument(
        '--hidden-dims',
        type=str,
        default='512,512',
        help='Hidden dimensions (default: 512,512)'
    )
    parser.add_argument(
        '--save-path',
        type=str,
