...
class PolicyValueLoss:
    ...
    def setup_hierarchical_weights(
        self,
        action_type_weights: torch.Tensor,
        param_weights: Dict[int, torch.Tensor]
    ):
        """
        ...
        self.action_type_criterion = nn.CrossEntropyLoss(
            weight=action_type_weights.to(self.device)
        )
    ...
    def compute_hierarchical_loss(
        ...
        for action_type_idx, samples in target_param_indices.items():
            if action_type_idx in param_logits:
                ...
                param_targets = torch.tensor([s[1] for s in samples], dtype=torch.long, device=self.device)
                ...
                if action_type_idx in self.param_criteria:
                    param_loss += self.param_criteria[action_type_idx](logits_for_type, param_targets) * len(samples)
                else:
...
def create_loss_computer(
    ...
    if model_type == 'flat':
        ...
        if use_class_weights and train_actions is not None:
            ...
            print(f"  Action type weight range: [{action_type_weights.min():.3f}, {action_type_weights.max():.3f}]")
