...
if TYPE_CHECKING:
    from catanrl.envs.zoo.multi_env import MultiAgentCatanatronEnv
...
class RewardFunction:
    ...
    def reward(
        self,
        env: "MultiAgentCatanatronEnv",
        game: Game,
        color: Color,
    ) -> float:
        raise NotImplementedError

    def after_step(self, env: "MultiAgentCatanatronEnv", game: Game, color: Color) -> None:
        return None

    def after_reset(self, env: "MultiAgentCatanatronEnv") -> None:
        return None
...
class ShapedReward(RewardFunction):
    def __init__(
        ...
        self.prev_production: Dict[Color, int] = {color: 0 for color in env.colors_order}
    ...
    def reward(
        ...
        production_diff = (
            sum(build_production_features(True)(game, color).values()) - self.prev_production[color]
        )
        reward = (
            self.config["vp_diff_weight"] * (vp_diff / env.config.vps_to_win)
    ...
    def after_step(
        ...
        self.prev_vps[color] = get_actual_victory_points(game.state, color)
        self.prev_production[color] = sum(build_production_features(True)(game, color).values())
