...
_DICE_PROBABILITY = {number: (6 - abs(7 - number)) / 36.0 for number in range(2, 13)}
...
def native_numeric_features(
    ...
    for relative_index in range(game.num_players):
        ...
        _add_player_features(features, relative_index, game.player_state(absolute_index))
...
def full_native_features(game: NativeGame, map_type: MapType, base_player: int) -> np.ndarray:
